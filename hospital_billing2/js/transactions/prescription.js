...
class PrescriptionManager {
  ...
  async loadPrescriptions() {
    ...
    try {
      ...
      tbody.innerHTML = `<tr><td colspan="7" class="text-center text-danger py-4">${this.escape(err.message)}</td></tr>`;
    }
  }
  ...
  async loadMedicines() {
    try {
      const res = await axios.get(BASE_API_URL + 'medicine.php', { params: { operation: 'getAllMedicines' } });
            ...
            <select id="medicineId" class="form-select" required>
              ...
              ${this.medicines.map(m => `
                <option value="${this.escape(m.medicineid)}" ${item && item.medicineid == m.medicineid ? 'selected' : ''}>
                  ${this.escape(m.brand_name || m.name || '')}
                </option>`).join('')}
            </select>
