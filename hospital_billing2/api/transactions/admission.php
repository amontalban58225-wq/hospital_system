...
class AdmissionAPI {
    ...
    private function mapStatusUI($status): array {
        $map = [
            'Admitted' => ['label' => 'Admitted', 'class' => 'badge bg-success text-white'],
            'Discharged' => ['label' => 'Discharged', 'class' => 'badge bg-danger text-white'],
            'Closed' => ['label' => 'Closed', 'class' => 'badge bg-secondary text-white'],
        ];
        return $map[$status] ?? ['label' => $status, 'class' => 'badge bg-primary text-white'];
    }
    ...
    public function getAllAdmissions() {
        try {
            ...
            $this->respond(['error' => 'Failed to fetch admissions: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function getActiveAdmissions() {
        try {
            ...
            $this->respond(['error' => 'Failed to fetch active admissions: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function insertAdmission($data) {
        try {
            ...
            $sql = "INSERT INTO Admission (patientid, doctorid, userid, admission_date) VALUES (:patientid, :doctorid, :userid, :admission_date)";
            $stmt = $this->conn->prepare($sql);
            $stmt->execute([
                ':patientid' => $patientid,
                ':doctorid' => $doctorid,
            ...
            $this->respond(['success' => false, 'error' => 'Error inserting admission: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function updateAdmission($data) {
        try {
            ...
            $sql = "UPDATE Admission SET patientid = :patientid, doctorid = :doctorid, userid = :userid, admission_date = :admission_date, status = :status WHERE admissionid = :admissionid";
            $stmt = $this->conn->prepare($sql);
            $stmt->execute([
            ...
            $this->respond(['success' => false, 'error' => 'Error updating admission: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function deleteAdmission($admissionid) {
        try {
            if (empty($admissionid))
                $this->respond(['success' => false, 'error' => 'Admission ID is required'], 422);
            ...
            $this->respond(['success' => false, 'error' => 'Error deleting admission: ' . $e->getMessage()], 500);
        }
    }
}
